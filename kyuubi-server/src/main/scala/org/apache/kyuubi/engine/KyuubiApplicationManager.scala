...
class KyuubiApplicationManager extends AbstractService("KyuubiApplicationManager") {
  ...
  def killApplication(
    ...
    for (operation <- operations if !killed) {
      if (operation.isSupported(resourceManager)) {
        val (k, m) = operation.killApplicationByTag(tag, cluster, group, resourceType)
        killed = k
        lastMessage = m
      }
    }
  ...
  def getApplicationInfo(
      ...
      cluster: Option[Cluster] = None,
      group: Option[Group] = None,
      resourceType: Option[ResourceType] = Some(POD)): Option[ApplicationInfo] = {
    debug(
      s"GetApplicationInfo clusterManager: [$clusterManager], tag: [$tag], " +
        s"cluster: [${cluster.map(a => a.getId)}]")
    val operation = operations.find(_.isSupported(clusterManager))
    operation match {
      case Some(op) => Option(op.getApplicationInfoByTag(tag, cluster, group, resourceType))
      case None => None
    }
  }

  def getApplicationLog(
      clusterManager: Option[String],
      tag: String,
      cluster: Option[Cluster] = None,
      group: Option[Group] = None): Option[InputStream] = {
    debug(
      s"GetApplicationLog clusterManager: [$clusterManager], tag: [$tag], " +
        s"cluster: [${cluster.map(a => a.getId)}]")
    val operation = operations.find(_.isSupported(clusterManager))
    operation match {
      case Some(op) => Option(op.getApplicationLogByTag(tag, cluster, group))
      case None => None
    }
  }
}
...
object KyuubiApplicationManager {
  ...
  private def setupSparkK8sTag(tag: String, conf: KyuubiConf): Unit = {
    ...
    conf.set("spark.kubernetes.driver.label." + LABEL_KYUUBI_APP_KEY, LABEL_KYUUBI_APP_VALUE)
  }
  ...
  private def setupFlinkK8sTag(tag: String, conf: KyuubiConf): Unit = {
    val originalTag = conf.getOption(FlinkProcessBuilder.K8S_LABEL_KEY).map(_ + ",").getOrElse("")
    val newTag =
      s"$originalTag$LABEL_KYUUBI_UNIQUE_KEY:$tag,$LABEL_KYUUBI_APP_KEY:$LABEL_KYUUBI_APP_VALUE"
    conf.set(FlinkProcessBuilder.K8S_LABEL_KEY, newTag)
  }
  ...
  def tagApplication(
    ...
    (applicationType.toUpperCase, resourceManager.map(_.toUpperCase())) match {
      ...
      case ("FLINK", Some("YARN")) => setupFlinkYarnTag(applicationTag, conf)
      case ("FLINK", Some(rm)) if rm.startsWith("K8S") => setupFlinkK8sTag(applicationTag, conf)
