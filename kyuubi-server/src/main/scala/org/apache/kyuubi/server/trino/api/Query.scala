...
import org.apache.kyuubi.config.KyuubiConf.KYUUBI_SESSION_CLUSTER_TAGS
...
object Query {
  ...
  def apply(
    ...
    val sessionHandle = getOrCreateSession(host, context, backendService)
  ...
  private def getOrCreateSession(
    ...
    context.session.get(KYUUBI_SESSION_ID).map(SessionHandle.fromUUID).getOrElse {
      ...
      trinoInfo.put(KYUUBI_SESSION_CLUSTER_TAGS.key, s"host:$host")

      // override host parsed tags if needed
      if (context.session.nonEmpty && context.session.contains("cluster.tags")) {
        trinoInfo.put(
          KYUUBI_SESSION_CLUSTER_TAGS.key,
          context.session.getOrElse("cluster.tags", ""))
      }

      trinoInfo.put(
        KyuubiConf.ENGINE_TRINO_CONNECTION_CATALOG.key,
        context.catalog.getOrElse("hive"))

      trinoInfo.put(
        KyuubiConf.ENGINE_JDBC_CONNECTION_DATABASE.key,
        context.schema.getOrElse(KyuubiConf.ENGINE_JDBC_CONNECTION_DATABASE.defaultValStr))

      trinoInfo.put(KyuubiConf.ENGINE_TYPE.key, "trino")
