...
import javax.ws.rs.core.{HttpHeaders, Response, UriInfo}
...
import org.apache.kyuubi.util.CompressionUtil
...
object TrinoContext extends Logging {
  ...
  val FILTER_HEADER: Seq[String] =
    Seq("Authorization", "Accept-Encoding", "Host", TRINO_HEADERS.requestSource)
  ...
  def apply(headers: Map[String, List[String]]): TrinoContext = {
    val agent = headers.getOrElse("User-Agent", Seq("Trino")).head
    if (agent.contains("Presto")) {
      return PrestoContext(headers)
    }
  ...
  def buildTrinoHeader(headers: HttpHeaders): Map[String, Seq[String]] = {
    PrestoContext.buildTrinoHeader(headers: HttpHeaders).filter(kv =>
      !FILTER_HEADER.contains(kv._1)) ++ Map(
      TRINO_HEADERS.requestSource -> Seq("kyuubi"))
  }

  def buildTrinoResponse(qr: QueryResults, trinoContext: TrinoContext): Response = {
    buildTrinoResponse(qr.toString, trinoContext)
  }

  def buildTrinoResponse(data: String, trinoContext: TrinoContext): Response = {
    val responseBuilder = Response.ok(data)
  ...
  private def buildResponseData(header: Map[String, Seq[String]], data: String): Any = {
    var compressed = false
    if (header.contains("content-encoding")) {
      header.getOrElse("content-encoding", Seq()).foreach { h =>
    ...
    if (compressed) {
      CompressionUtil.compressByGzip(data)
    } else {
      data
    }
  }

  def buildTrinoResponse(
      header: Map[String, Seq[String]],
      body: String,
      uriInfo: UriInfo,
      trinoContext: TrinoContext): Response = {
    val data = replaceNextUri(body, uriInfo)
    val responseBuilder = Response.ok(buildResponseData(header, data))
    header.foreach {
      case (k, v) => v.foreach { v0 =>
          responseBuilder.header(k, v0)
        }
    }
    trinoContext.preparedStatement.foreach {
      case (k, v) =>
  ...
  def replaceSchemeAndHost(uri: URI, replaceScheme: String, replaceHost: String): String = {
    ...
    uri.toString.replaceFirst(scheme, replaceScheme).replaceFirst(host, replaceHost)
  }

  private def replaceNextUri(result: String, uriInfo: UriInfo): String = {
    val json = JSONObject.parseObject(result)
    val scheme = uriInfo.getRequestUri.getScheme
    var host = uriInfo.getRequestUri.getHost
    val port = uriInfo.getRequestUri.getPort
    if (port > 0) {
      host = host + ":" + port
    }

    val infoUri = json.getString("infoUri")
    if (null != infoUri) {
      json.put("infoUri", replaceSchemeAndHost(URI.create(infoUri), scheme, host))
    }
    val partialCancelUri = json.getString("partialCancelUri")
    if (null != partialCancelUri) {
      json.put("partialCancelUri", replaceSchemeAndHost(URI.create(partialCancelUri), scheme, host))
    }
    val nextUri = json.getString("nextUri")
    if (null != nextUri) {
      json.put("nextUri", replaceSchemeAndHost(URI.create(nextUri), scheme, host))
    }
    json.toJSONString()
  }
