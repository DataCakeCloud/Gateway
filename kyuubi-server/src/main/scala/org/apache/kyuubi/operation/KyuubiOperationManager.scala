...
import org.apache.kyuubi.session.jdbc.KyuubiJDBCOperation
import org.apache.kyuubi.session.trino.KyuubiTrinoOperation
...
class KyuubiOperationManager private (name: String) extends OperationManager(name) {
  ...
  def newExecuteSyncStatementOperation(
      session: Session,
      statement: String,
      confOverlay: Map[String, String],
      runAsync: Boolean,
      ...
      incrementalCollect: Boolean = false): Operation = {
    val operation = engineType match {
      case EngineType.TRINO =>
        new KyuubiTrinoOperation(
          session,
          statement,
          confOverlay,
          runAsync,
  ...
  def newBatchJobSubmissionOperation(
      ...
      cluster: Option[Cluster] = None): BatchJobSubmission = {
    val operation = Option(batchType).map(_.toUpperCase(Locale.ROOT)) match {
      case Some("SCRIPT") | Some("TFJOB") =>
        new ScriptJobSubmission(
          session,
          batchType,
          batchName,
          resource,
