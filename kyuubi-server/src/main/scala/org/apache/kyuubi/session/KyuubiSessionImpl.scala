...
import org.apache.kyuubi.engine.{ApplicationState, EngineRef, EngineType, KyuubiApplicationManager}
...
class KyuubiSessionImpl(
  ...
  private val dynamic = sessionManager.clusterDynamic && EngineType.SPARK_SQL ==
    EngineType.withName(sessionConf.get(ENGINE_TYPE))

  val cluster: Option[Cluster] = selectCluster(sessionConf, dynamic = dynamic)
  ...
  override def close(): Unit = {
    ...
    try {
      ...
      // add by smy, kill engine when engine terminated
