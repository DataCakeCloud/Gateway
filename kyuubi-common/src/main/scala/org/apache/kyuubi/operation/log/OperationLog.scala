...
object OperationLog extends Logging {
  ...
  def findOperationLog(operationLogRoot: String, engineRefId: String): OperationLog = {
    if (null == operationLogRoot || null == engineRefId) {
      return null
    }
    val operationLogPath = Paths.get(operationLogRoot, engineRefId)
    if (!Files.exists(operationLogPath)) {
      Files.createDirectories(operationLogPath)
    }
    val opLog =
      if (Files.list(operationLogPath).count() > 0) {
        new OperationLog(Files.list(operationLogPath).findFirst().get())
      } else {
        // double id
        new OperationLog(Files.createFile(Paths.get(operationLogRoot, engineRefId, engineRefId)))
      }
    opLog.initOperationLogIfNecessary()
    opLog
  }
...
class OperationLog(path: Path) {
  ...
  def close(): Unit = synchronized {
    ...
    /* Don`t delete operation log at here by smy
    trySafely {
      Files.delete(path)
    }
     */
