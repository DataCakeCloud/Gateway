...
object KyuubiConf {
  ...
  final val KYUUBI_CONF_SEPARATE_COMMA = ","
  final val KYUUBI_CONF_SEPARATE_SEMICOLON = ";"
  ...
  val ENGINE_TRINO_SERVER_NAME: OptionalConfigEntry[String] =
    buildConf("kyuubi.session.engine.trino.server.name")
      .doc("The server name that Trino engine will connect to")
      .version("1.7.0")
      .stringConf
      .createOptional
  ...
  val KYUUBI_CLUSTER_CONFIG_PATH: ConfigEntry[String] =
    buildConf("kyuubi.cluster.config.path")
      .doc("Kyuubi custom config path for cluster selector.")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_CLUSTER_NAME: ConfigEntry[String] =
    buildConf("kyuubi.cluster.name")
      .doc("Kyuubi cluster name.")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_SESSION_CLUSTER_TAGS: ConfigEntry[Seq[String]] =
    buildConf("kyuubi.session.cluster.tags")
      .doc("A comma-separated list of cluster tags.")
      .version("1.7.0")
      .stringConf
      .toSequence()
      .createWithDefault(Seq())

  val KYUUBI_SESSION_COMMAND_TAGS: ConfigEntry[Seq[String]] =
    buildConf("kyuubi.session.command.tags")
      .doc("A comma-separated list of command tags.")
      .version("1.7.0")
      .stringConf
      .toSequence()
      .createWithDefault(Seq())

  val KYUUBI_SESSION_CLUSTER_SELECTOR_NAMES: ConfigEntry[Seq[String]] =
    buildConf("kyuubi.session.cluster.selector.names")
      .doc("A comma-separated list of cluster selector names.")
      .version("1.7.0")
      .stringConf
      .toSequence()
      .createWithDefault(Seq("priority", "usage", "random"))

  val KYUUBI_SESSION_RETRY_TIMES: ConfigEntry[Int] =
    buildConf("kyuubi.session.retry.times")
      .doc("Session current retry times.")
      .version("1.7.0")
      .intConf
      .createWithDefault(1)

  val BATCH_APPLICATION_PENDING_TIMEOUT: ConfigEntry[Long] =
    buildConf("kyuubi.batch.application.pending.timeout")
      .doc("Threshold above which to warn batch application may be pending.")
      .version("1.7.0")
      .timeConf
      .createWithDefault(Duration.ofHours(1).toMillis)

  val KYUUBI_BATCH_COMMAND_NAME: ConfigEntry[String] =
    buildConf("kyuubi.batch.command.name")
      .doc("Kyuubi job command name.")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_SESSION_OUTPUT: ConfigEntry[String] =
    buildConf("kyuubi.session.output")
      .doc("The session output s3 or obs path")
      .version("1.7.0")
      .stringConf
      .createWithDefault(null)

  val KYUUBI_SESSION_OUTPUT_REGION: ConfigEntry[String] =
    buildConf("kyuubi.session.output.region")
      .doc("The session output s3 or obs region")
      .version("1.7.0")
      .stringConf
      .createWithDefault(null)

  val KYUUBI_SESSION_OUTPUT_FILENAME: ConfigEntry[String] =
    buildConf("kyuubi.session.output.filename")
      .doc("The session output s3 or obs filename")
      .version("1.7.0")
      .stringConf
      .createWithDefault("00000000")

  val KYUUBI_SESSION_OUTPUT_FORMAT: ConfigEntry[String] =
    buildConf("kyuubi.session.output.format")
      .doc("The session output s3 or obs filename")
      .version("1.7.0")
      .stringConf
      .createWithDefault("csv")

  val KYUUBI_SESSION_OUTPUT_DELIMITER: ConfigEntry[String] =
    buildConf("kyuubi.session.output.delimiter")
      .doc("The session output column delimiter")
      .version("1.7.0")
      .stringConf
      .createWithDefault(",")

  val KYUUBI_SESSION_OUTPUT_FILE_SIZE: ConfigEntry[Long] =
    buildConf("kyuubi.session.output.fileSize")
      .doc("The session output single file max size, default 100 MB")
      .version("1.7.0")
      .longConf
      .createWithDefault(100 * 1024 * 1024L)

  val ENGINE_JDBC_CONNECTION_DATABASE: ConfigEntry[String] =
    buildConf("kyuubi.engine.jdbc.connection.database")
      .doc("The engine connection database")
      .version("1.7.0")
      .stringConf
      .createWithDefault("default")

  val ENGINE_FLINK_APPLICATION_MODE: ConfigEntry[String] =
    buildConf("kyuubi.session.engine.flink.application.mode")
      .doc("The package used to create Flink engine remote job. If it is undefined," +
        " Kyuubi will use the default")
      .version("1.7.0")
      .stringConf
      .createWithDefault("run-application")

  val KYUUBI_S3_REGION: ConfigEntry[String] =
    buildConf("kyuubi.s3.region")
      .doc("s3 region")
      .version("1.7.0")
      .stringConf
      .createWithDefault(null)

  val KYUUBI_OBS_ENDPOINT: ConfigEntry[String] =
    buildConf("kyuubi.obs.endpoint")
      .doc("obs endpoint")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_OBS_ACCESS_KEY: ConfigEntry[String] =
    buildConf("kyuubi.obs.accessKey")
      .doc("obs access key")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_OBS_SECRET_KEY: ConfigEntry[String] =
    buildConf("kyuubi.obs.secretKey")
      .doc("obs secret key")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_KS3_ENDPOINT: ConfigEntry[String] =
    buildConf("kyuubi.ks3.endpoint")
      .doc("ks3 endpoint")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_KS3_ACCESS_KEY: ConfigEntry[String] =
    buildConf("kyuubi.ks3.accessKey")
      .doc("ks3 access key")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_KS3_SECRET_KEY: ConfigEntry[String] =
    buildConf("kyuubi.ks3.secretKey")
      .doc("ks3 secret key")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_OSS_ENDPOINT: ConfigEntry[String] =
    buildConf("kyuubi.oss.endpoint")
      .doc("oss endpoint")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_OSS_ACCESS_KEY: ConfigEntry[String] =
    buildConf("kyuubi.oss.accessKey")
      .doc("oss access key")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_OSS_SECRET_KEY: ConfigEntry[String] =
    buildConf("kyuubi.oss.secretKey")
      .doc("oss secret key")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_SESSION_SELECT_CLUSTER_DYNAMIC: ConfigEntry[Boolean] =
    buildConf("kyuubi.session.select.cluster.dynamic")
      .doc("The session select cluster dynamic")
      .version("1.7.0")
      .booleanConf
      .createWithDefault(false)

  val KYUUBI_AUTHENTICATION_ENABLED: ConfigEntry[Boolean] =
    buildConf("kyuubi.authentication.enabled")
      .doc("The authentication enabled")
      .version("1.7.0")
      .booleanConf
      .createWithDefault(false)

  val KYUUBI_KRB5_DEBUG: ConfigEntry[Boolean] =
    buildConf("kyuubi.kerberos.debug")
      .doc("The kerberos debug enabled")
      .version("1.7.0")
      .booleanConf
      .createWithDefault(false)

  val KYUUBI_KRB5_CONF_PATH: ConfigEntry[String] =
    buildConf("kyuubi.krb5.conf.path")
      .doc("The authentication kerberos conf file path")
      .version("1.7.0")
      .stringConf
      .createWithDefault("/etc/krb5.conf")

  val KYUUBI_HIVE_SERVER_PRINCIPAL: ConfigEntry[String] =
    buildConf("kyuubi.hive.server.principal")
      .doc("Name of the HiveServer Kerberos principal.")
      .version("1.0.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_AUTHORIZATION_ENABLED: ConfigEntry[Boolean] =
    buildConf("kyuubi.authorization.enabled")
      .doc("The authorization enabled")
      .version("1.7.0")
      .booleanConf
      .createWithDefault(false)

  val KYUUBI_KINIT_PATH: ConfigEntry[String] =
    buildConf("kyuubi.kinit.path")
      .doc("Location of kinit path.")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_PRINCIPAL_PATTERN: ConfigEntry[String] =
    buildConf("kyuubi.principal.pattern")
      .doc("The kerberos principal pattern.")
      .version("1.7.0")
      .stringConf
      .createWithDefault("${USER}/${SERVICE}@${HOST}")

  val KYUUBI_SESSION_GROUP: ConfigEntry[String] =
    buildConf("kyuubi.session.group")
      .doc("The session group")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_SESSION_GROUP_ID: ConfigEntry[String] =
    buildConf("kyuubi.session.groupId")
      .doc("The session group id")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_SESSION_KEYTAB_PREFIX: ConfigEntry[String] =
    buildConf("kyuubi.session.keytab.prefix")
      .doc("The session keytab prefix")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_SESSION_PRINCIPAL_HOST: ConfigEntry[String] =
    buildConf("kyuubi.session.principal.host")
      .doc("The session principal host")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_SESSION_PRINCIPAL_REALM: ConfigEntry[String] =
    buildConf("kyuubi.session.principal.realm")
      .doc("The session principal realm")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_SESSION_TENANT: ConfigEntry[String] =
    buildConf("kyuubi.session.tenant")
      .doc("The session tenant")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_SESSION_MODE: ConfigEntry[String] =
    buildConf("kyuubi.session.mode")
      .doc("The session mode process or thread")
      .version("1.7.0")
      .stringConf
      .createWithDefault("process")

  // ///////////////////////////////////////////////////////////////////////////////////////////////
  //                                 LakeCat config begin                                         //
  // ///////////////////////////////////////////////////////////////////////////////////////////////

  val KYUUBI_LAKECAT_CLIENT_HOST: ConfigEntry[String] =
    buildConf("kyuubi.lakecat.client.host")
      .doc("lakecat client host.")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  val KYUUBI_LAKECAT_CLIENT_PORT: ConfigEntry[Int] =
    buildConf("kyuubi.lakecat.client.port")
      .doc("lakecat client port.")
      .version("1.7.0")
      .intConf
      .createWithDefault(80)

  val KYUUBI_LAKECAT_CLIENT_TOKEN: ConfigEntry[String] =
    buildConf("kyuubi.lakecat.client.token")
      .doc("lakecat client token.")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")
  ...
  val KYUUBI_ENGINE_LOG_UPLOAD_PREFIX: ConfigEntry[String] =
    buildConf("kyuubi.engine.log.upload.prefix")
      .doc("kyuubi engine log upload object storage prefix.")
      .version("1.7.0")
      .stringConf
      .createWithDefault("")

  // storage thread pool config
  val STORAGE_EXEC_POOL_SIZE: ConfigEntry[Int] =
    buildConf("kyuubi.storage.exec.pool.size")
      .doc("Number of threads in the operation execution thread pool of Kyuubi server for storage")
      .version("1.0.0")
      .intConf
      .createWithDefault(200)

  val STORAGE_EXEC_POOL_MAX_SIZE: ConfigEntry[Int] =
    buildConf("kyuubi.storage.exec.pool.max.size")
      .doc(
        "Number of max threads in the operation execution thread pool of Kyuubi server for storage")
      .version("1.0.0")
      .intConf
      .createWithDefault(400)

  val STORAGE_EXEC_WAIT_QUEUE_SIZE: ConfigEntry[Int] =
    buildConf("kyuubi.storage.exec.pool.wait.queue.size")
      .doc("Size of the wait queue for the operation execution thread pool of Kyuubi server " +
        "for storage")
      .version("1.0.0")
      .fallbackConf(SERVER_EXEC_WAIT_QUEUE_SIZE)

  val STORAGE_EXEC_KEEPALIVE_TIME: ConfigEntry[Long] =
    buildConf("kyuubi.storage.exec.pool.keepalive.time")
      .doc("Time(ms) that an idle async thread of the operation execution thread pool will wait" +
        " for a new task to arrive before terminating in Kyuubi server for storage")
      .version("1.0.0")
      .fallbackConf(SERVER_EXEC_KEEPALIVE_TIME)

  val KYUUBI_TRINO_API_AUTHENTICATION_ENABLED: ConfigEntry[Boolean] =
    buildConf("kyuubi.trino.api.authentication.enabled")
      .doc("The trino api authentication enabled")
      .version("1.7.0")
      .booleanConf
      .createWithDefault(false)
}
